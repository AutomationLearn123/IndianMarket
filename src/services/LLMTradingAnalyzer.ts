/**
 * 🤖 LLM TRADING ANALYZER SERVICE (TypeScript)
...
export class LLMTradingAnalyzer {
  
  async generateTradingSignal(marketData: TradingSignalRequest): Promise<TradingSignal> {
    try {
  ...
  private buildAnalysisPrompt(data: TradingSignalRequest): string {
...
${recentCandles.map((candle, i) => 
  `${i+1}. OHLC: ${candle[1]}/${candle[2]}/${candle[3]}/${candle[4]} | Vol: ${candle[5].toLocaleString()}`
).join('\n')}
...
`;
  ...
  private parseSignalFromAnalysis(analysis: string, marketData: TradingSignalRequest): TradingSignal {
    try {
  ...
  private extractAction(analysis: string, jsonAction?: string): 'BUY' | 'SELL' | 'HOLD' | 'NO_TRADE' {
    if (jsonAction) return jsonAction as any;
    
    const upperAnalysis = analysis.toUpperCase();
    if (upperAnalysis.includes('BUY') && !upperAnalysis.includes("DON'T BUY")) return 'BUY';
    if (upperAnalysis.includes('SELL') && !upperAnalysis.includes("DON'T SELL")) return 'SELL';
    if (upperAnalysis.includes('HOLD')) return 'HOLD';
    return 'NO_TRADE';
  }
  
  private extractConfidence(analysis: string, jsonConfidence?: number): number {
    if (jsonConfidence !== undefined) return Math.max(0, Math.min(1, jsonConfidence));
  ...
  private calculateStopLoss(entryPrice: number, action: string): number {
  ...
  private calculateTarget(entryPrice: number, stopLoss: number, action: string): number {
  ...
  private calculateRiskReward(entryPrice: number, stopLoss: number, target: number): number {
    const risk = Math.abs(entryPrice - stopLoss);
    const reward = Math.abs(target - entryPrice);
    return reward / risk;
  }
  
  private getDefaultSignal(marketData: TradingSignalRequest): TradingSignal {
    return {
      symbol: marketData.symbol,
      action: 'NO_TRADE',
      confidence: 0,
  ...
  private generateMockSignal(marketData: TradingSignalRequest): TradingSignal {
    ...
    let action: 'BUY' | 'SELL' | 'HOLD' | 'NO_TRADE' = 'HOLD';
    let confidence = 0.3;
...
export const llmAnalyzer = new LLMTradingAnalyzer();
