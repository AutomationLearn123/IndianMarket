...
export interface AppConfig {
  ...
  // File Upload Configuration (legacy - may remove)
  maxFileSize: number;
  allowedFileTypes: string[];
  uploadDir: string;
  
  // Trading Configuration
  minVolumeRatio: number; // Minimum volume ratio for signal (e.g., 4.0 for 400%)
  minConfidence: number; // Minimum confidence for signals
...
const validateRequiredEnvVars = (): void => {
  ...
  const hasPlaceholders = placeholders.filter(
    ({ key, placeholder }) => process.env[key] === placeholder
  );
  
  if (hasPlaceholders.length > 0) {
    logger.warn(
      `Using placeholder values for: ${hasPlaceholders.map(p => p.key).join(', ')}. ` +
...
const parseCorsOrigins = (origins?: string): string[] => {
  if (!origins) return defaultConfig.corsOrigin || [];
 ...
 */
const parseAllowedFileTypes = (types?: string): string[] => {
  if (!types) return defaultConfig.allowedFileTypes || [];
